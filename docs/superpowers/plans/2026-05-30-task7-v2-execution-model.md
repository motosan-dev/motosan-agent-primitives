...
- **A defer must be VISIBLE in the canonical pending set BEFORE its event is observable (A.2 race finding).** There is a window in the per-call future between emitting the defer event (inside `intercept_tool_call`) and registering its waiter; a synchronous `AskUserAnswer` (esp. wildcard) can hit the ops loop in that window, so canonicalization would see an incomplete pending set and target the wrong call (and the explicit deliver-before-register is caught by the WaiterRegistry buffer, A.1). Fix: a deferring call must **mark itself in the canonical pending set (and register its waiter) before — or atomically with — the event being emitted** (e.g. register/mark first, then emit; or mark pending synchronously at the point `intercept_tool_call` decides to defer). The wildcard canonicalization then always sees the complete set. **Add a deterministic test:** call #2 already awaiting; an extension emits the question for canonical #1 and the callback sends a wildcard answer BEFORE #1's waiter registers — assert the wildcard targets #1 (canonical), not #2.

...
- [ ] **A.3 — streaming onto `run_tool_call` (this is where the old path finally dies).** Migrate the streaming executors (`run_streaming_with_cancel_inner`, `run_streaming_inner`, `run_streaming_inner_with_cancel_and_ops`) off the two-phase slot mechanism onto `run_tool_call` + `nonstreaming_ops_loop` (rename it once it's no longer non-streaming-only). Then **delete the now-dead old path**: `resolve_deferred_slots`, `execute_tools_parallel`, `resolve_and_execute_intercepted_slots`, `resolve_and_combine_preexecuted_slots`, and the `InterceptedSlot::{Deferred, DeferredPermission}` variants — these survive A.2 ONLY because streaming still uses them. Also drop the `#[allow(dead_code)]` on `WaiterRegistry::register_and_wait` (or remove the combined helper if the split register/wait is the only caller). Existing streaming tests pass unchanged. Commit.
